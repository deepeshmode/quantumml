% A0 portrait conference poster.
%
%   pdflatex poster.tex        (run twice)
%
% Needs in the same directory: fig_pipeline.png, fig_scaling.png, qr_repo.png
%
...
% prose not bullets, no boxes or rules around sections, three colours, one
% sans-serif family, title set as a conclusion.
%
% TUNING: if the columns overrun the sheet, reduce \figbandwidth first, then
% \bodysize. Never reduce the margins - white space is doing work.
...
\setbeamersize{text margin left=64mm, text margin right=64mm}
...
\newlength{\colw}
\setlength{\colw}{195mm}                       % (841 - 2*64 - 2*64) / 3
\newlength{\figbandwidth}
\setlength{\figbandwidth}{660mm}

% Section heading: coloured, never boxed, generous space above.
\newcommand{\shead}[1]{%
  \par\vspace{14mm}\noindent{\headsize\bfseries\color{cq}#1}\par\vspace{5mm}}
\newcommand{\sheadtop}[1]{%
  \par\noindent{\headsize\bfseries\color{cq}#1}\par\vspace{5mm}}
\newcommand{\figcap}[1]{\par\vspace{4mm}{\capsize\color{ink2}#1}\par}
...
\vspace{10mm}
{\authsize Deepesh V. Chaudhari\\[3mm]
...
\begin{center}
\begin{tikzpicture}[
  sb/.style={text width=148mm, align=left, rounded corners=6mm,
...
\begin{center}
  \includegraphics[width=\colw]{fig_scaling.png}
\end{center}
...
\textperiodcentered{} Krawec, FAS \textperiodcentered{} EPRI.\\[3mm]
Code: github.com/deepeshmode/quantumml\\[3mm]
With thanks to Siwei Qiu.\par}
\end{minipage}\hfill
\begin{minipage}[b]{0.34\colw}
\raggedleft\includegraphics[width=0.9\linewidth]{qr_repo.png}
\end{minipage}
